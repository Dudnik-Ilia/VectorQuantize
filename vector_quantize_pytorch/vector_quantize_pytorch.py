...
def gumbel_sample(
    ...
    # Finally choosing the Max -log(dist) == Min dist embedding
    ind = sampling_logits.argmax(dim=dim)
    one_hot = F.one_hot(ind, size).type(dtype)

    assert not (
                reinmax and not straight_through), 'reinmax can only be turned on if using straight through gumbel ' \
                                                   'softmax'

    if not straight_through or temperature <= 0. or not training:
        return ind, one_hot

    # use reinmax for better second-order accuracy - https://arxiv.org/abs/2304.08612 [17 Apr 2023]
    # Brand new algorithm 2
...
class EuclideanCodebook(nn.Module):
    ...
    def expire_codes_(self, batch_samples):
        ...
        if not torch.any(expired_codes):
            return
        # If the code is not being used often, replace with the one from batch
    ...
    def forward(self, x,
                ...
                mask=None):
        # Add Num_Codebooks_Dim at the beginning if needed
        needs_codebook_dim = x.ndim < 4
        sample_codebook_temp = default(sample_codebook_temp, self.sample_codebook_temp) # Temperature

        x = x.float()

        if needs_codebook_dim:
            x = rearrange(x, '... -> 1 ...')
        # Pack all the columns of all the pictures together in one dim
        dtype = x.dtype
        ...
        # This is Tensor and if we do not learn it,
        # then detach from the computational graph of derivatives propagation
        # --> requires_grad = False, then update manually via steps
        ...
        # Calculating distances between image columns and embeddings
        # p = 2 stands for l2; minus is used to transform to logits
        dist = -torch.cdist(flatten, embed, p=2)

        # Sampling procedure, to choose the embedding (might add noise),
        # By default no Noise, choosing the Max -log(dist) == Min
        embed_ind, embed_onehot = self.gumbel_sample(dist, dim=-1, temperature=sample_codebook_temp,
                                                     training=self.training)
        # Unpack all columns together -> columns per image
        embed_ind = unpack_one(embed_ind, ps, 'h *')

        if self.training:
            unpacked_onehot = unpack_one(embed_onehot, ps, 'h * c')
            # Choosing the embedding from codebook, as if indexing in table
            quantize = einsum('h b n c, h c d -> h b n d', unpacked_onehot, embed)
        else:
            quantize = batched_embedding(embed_ind, embed)
        # Update step for embeddings
        if self.training and self.ema_update:
            ...
            # Calculate how OFTEN embeddings were used for each lookup
            cluster_size = embed_onehot.sum(dim=1)

            self.all_reduce_fn(cluster_size)
            # Moving Percentile (average) for each code, how often used step update
            ema_inplace(self.cluster_size.data, cluster_size, self.decay)
            # Summing vectors that choose THIS EMBEDDING: [Heads, Codebook_size, Code_Dim]
            # in Dim will be summed vectors that chose it
            embed_sum = einsum('h n d, h n c -> h c d', flatten, embed_onehot)
            self.all_reduce_fn(embed_sum.contiguous())
            # Moving Percentile (average) for each code, what usually called this embedding
            ema_inplace(self.embed_avg.data, embed_sum, self.decay)
            # Smoothing to make numbers more uniform
            cluster_size = laplace_smoothing(self.cluster_size, self.codebook_size, self.eps) * self.cluster_size.sum(
                dim=-1, keepdim=True)
            # Finally updating codebook
...
class VectorQuantize(nn.Module):
    ...
    def forward(
        ...
        shape, device, heads, is_multiheaded, codebook_size, return_loss = \
            x.shape, x.device, self.heads, self.heads > 1, self.codebook_size, exists(indices)
        ...
        if self.training:
            # Turn off gradients, if we update manually vectors (loss for VQ)
            maybe_detach = torch.detach if not self.learnable_codebook else identity
            commit_quantize = maybe_detach(quantize)

            # straight through (loss for X)
            ...
            if self.commitment_weight > 0:
                if self.commitment_use_cross_entropy_loss:
                    ...
                    if exists(mask):
                        ...
                        # Commitment loss = difference btw X and X_q; Commit tensor grad is False!
